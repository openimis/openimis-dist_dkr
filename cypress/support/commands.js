...
Cypress.Commands.add('selectDropdown', (identifier, value) => {
  if (typeof identifier === 'number') {
    cy.get('[aria-haspopup="listbox"]').eq(identifier).click();
  } else {
...
Cypress.Commands.add('selectMultipleAutocomplete', (identifier, selections) => {
  selections.forEach((value) => {
    if (typeof identifier === 'number') {
      cy.get('.MuiAutocomplete-root input')
        .eq(identifier)
        .click()
        .type(value);
    } else {
...
Cypress.Commands.add('selectFirstAutocomplete', (identifier) => {
  if (typeof identifier === 'number') {
    cy.get('.MuiAutocomplete-root input')
      .eq(identifier)
      .click()
      .type(' ');
  } else if (identifier.startsWith('Search')) {
    cy.get(`input[placeholder="${identifier}"]`)
      .click()
      .type(' ');
  } else {
    cy.contains('label', identifier)
...
Cypress.Commands.add('delete', (tableLabel, code, buttonLabel) => {
  cy.contains(`${tableLabel}`)
    .closest('.MuiPaper-root')
    .within(() => {
      cy.contains('tr', code)
        .find(SELECTORS.deleteUserBtn)
        .click();
    });
...
Cypress.Commands.add('filterValue', (identifier, value) => {
  cy.contains('button', 'Reset filters').click({ force: true });
  if (value) {
    cy.chooseMuiAutocomplete(identifier, value);
  } else {
    cy.selectFirstAutocomplete(identifier);
  }
  cy.contains('button', 'Search').click({ force: true });
  cy.waitForGraphQL('search');
})
